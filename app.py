...
#                            ]
                            "args": [
                                ...
                                f"  CURL_EXIT_CODE=$?;" # <-- curlの終了コードを$?で取得
                                f"  if [ \"$CURL_EXIT_CODE\" -ne 0 ]; then"
                                f"    echo 'curl failed with exit code '$CURL_EXIT_CODE', waiting...';"
                                f"    sleep 5;"
                                f"  else"
                                ...
                                f"      exit 0; "
                                f"    else "
                                f"      echo 'Application returned status code '$STATUS_CODE', waiting...'; "
                                f"      sleep 5; "
                                f"    fi; "
                                f"  fi; "
                                f"done; "
